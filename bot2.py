...
stopuser = {}
token = '6829596047:AAHNTp27fEVLZLgI0Cejr6XriX6nIJiQNwQ'
	...
	def my_function():
	  ...
	  "timer": "none",
		...
		if BL == '𝗙𝗥𝗘𝗘':	
			...
			contact_button = types.InlineKeyboardButton(text="✨ 𝗢𝗪𝗡𝗘𝗥  ✨", url="https://t.me/W_4_8")
			keyboard.add(contact_button)
			random_number = random.randint(33, 82)
			photo_url = f'https://t.me/W_4_8/{random_number}'
...
𝒀𝑶𝑼𝑹 𝑷𝑳𝑨𝑵 𝑵𝑶𝑾 {BL}</b>
	''',reply_markup=keyboard)
			return
		keyboard = types.InlineKeyboardMarkup()
		contact_button = types.InlineKeyboardButton(text="✨ 𝗝𝗢𝗜𝗡 ✨", url="https://t.me/W_4_8")
		keyboard.add(contact_button)
...
𝗕𝗼𝘁 𝗕𝘆: @W_4_8</b>'''
					if "Funds" in last or 'Invalid postal' in last or 'avs' in last or 'added' in last or 'Duplicate' in last or 'Approved' in last or 'Card Issuer Declined CVV' in last:
						live += 1
...
def respond_to_vbv(message):
	def my_function():
		...
		try:
			...
			msg=f'''<b>𝗘𝗦𝗟𝗔𝗠 𝗖𝗛𝗞 𖣌 𝗩𝗜𝗣 𝗦𝗨𝗕𝗦𝗖𝗥𝗜𝗕𝗘𝗗 ✅
...
def start(message):
	def my_function():
		...
		try:
			...
			pas ='𝗘𝗦𝗟𝗔𝗠-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=4))
			current_time = datetime.now()
...
''',reply_markup=keyboard)
	...
	''',reply_markup=keyboard)
		...
		with open('data.json', 'w') as file:
			json.dump(json_data, file, indent=2)
		return
	ko = (bot.reply_to(message, "جارٍ الحصول على المعلومات.⌛").message_id)
	try:
		cc = message.reply_to_message.text
	except:
		cc=message.text
